...
const bucketName = "bobble-project";
...
app.post("/signup", (req, res) => {
  ...
  // form validations
  if (name.length < 3) {
    res.json({ alert: "Please Enter Name!" });
  } else if (!email.length) {
    res.json({ alert: "Please Enter Email!" });
...
app.post("/login", (req, res) => {
  ...
  if (!email.length || !password.length) {
    res.json({ alert: "Fill all the Fields" });
  }
...
app.post("/add-product", (req, res) => {
  ...
  if (!draft) {
    if (!name.length) {
      ...
      res.json({ alert: "Please enter a short description" });
    } else if (!price.length || !Number(price)) {
      res.json({ alert: "Enter valid price" });
    } else if (!detail.length) {
      res.json({ alert: "Please enter detail" });
    } else if (!tags.length) {
      res.json({ alert: "Enter the tags" });
    }
  }
